...
class User {
  ...
  static async findByEmail(email) {
    const [rows] = await db.query("SELECT * FROM NguoiDung WHERE Email = ?", [
      email,
    ]);
    return rows[0];
  }
  ...
  static async updatePassword(userId, newPassword) {
    ...
    await db.query("UPDATE NguoiDung SET MatKhau = ? WHERE MaNguoiDung = ?", [
      hashedPassword,
      userId,
    ]);
  }
  ...
  static async updateRole(userId, roleId) {
    await db.query("UPDATE NguoiDung SET VaiTroID = ? WHERE MaNguoiDung = ?", [
      roleId,
      userId,
    ]);
  }
