...
//khai báo component SalaryCalculation
const SalaryCalculation = () => {
  ...
  const [salaryResult, setSalaryResult] = useState(null); //kết quả tính lương
  ...
  const handleCalculate = async () => {
    ...
    try {
      ...
      const response = await api.get(
        `/salary/calculate/${formData.employeeId}/${formData.month}/${formData.year}`
      );
      setSalaryResult(response.data); // trả về bảng lương chi tiết
    } catch (error) {
  ...
  const handleSave = async () => {
    try {
      //gọi be để lưu bảng lương vado db
      await api.post(
        `/salary/save/${formData.employeeId}/${formData.month}/${formData.year}`
      );
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div>
            ...
            >
              ...
              {Array.isArray(employees) && //themdongnay
                employees.map((emp) => (
          ...
          <div className="p-6 space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="flex items-center justify-between p-3 bg-white rounded border">
                  ...
                  <span className="text-lg font-bold text-gray-900">
                    {formatCurrency(salaryResult.baseSalary)}
                  </span>
                </div>
                <div className="flex items-center justify-between p-3 bg-white rounded border">
                  <span className="font-medium text-gray-700">
                  ...
                  <span className="text-lg font-bold text-gray-900">
                    {salaryResult.actualWorkingDays}/26
                  </span>
                </div>
              </div>
            </div>
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between p-3 bg-white rounded border border-green-200">
                  ...
                  <span className="text-lg font-bold text-green-600">
                    {formatCurrency(salaryResult.grossSalary)}
                  </span>
                </div>
                <div className="flex items-center justify-between p-3 bg-white rounded border border-green-200">
                  <span className="font-medium text-gray-700">Thưởng:</span>
                  <span className="text-lg font-bold text-green-600">
                    {formatCurrency(salaryResult.totalReward)}
                  </span>
                </div>
                <div className="flex items-center justify-between p-3 bg-white rounded border border-green-200">
                  <span className="font-medium text-gray-700">Phạt:</span>
                  <span className="text-lg font-bold text-red-600">
                    -{formatCurrency(salaryResult.totalPenalty)}
                  </span>
                </div>
                  ...
                  <span className="text-xl font-bold text-green-700">
                    {formatCurrency(
                      salaryResult.grossSalary +
                        salaryResult.totalReward -
                        salaryResult.totalPenalty
                    )}
                  </span>
                </div>
              </div>
            ...
            <div className="bg-red-50 rounded-lg p-4">
              ...
              <div className="flex items-center justify-between p-3 bg-white rounded border border-red-200">
                <span className="font-medium text-gray-700">
                  Bảo hiểm (10.5% lương CB):
                </span>
                <span className="text-lg font-bold text-red-600">
                  -{formatCurrency(salaryResult.totalDeductions)}
                </span>
              </div>
            </div>
            ...
            <div className="bg-yellow-50 rounded-lg p-4 border-2 border-yellow-300">
              ...
              <div className="flex items-center justify-between p-4 bg-yellow-100 rounded border border-yellow-400">
                ...
                <span className="text-2xl font-bold text-yellow-700">
                  {formatCurrency(salaryResult.netSalary)}
                </span>
              </div>
            </div>
